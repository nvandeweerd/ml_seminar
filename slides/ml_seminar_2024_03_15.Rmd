...
xaringanExtra::use_banner(
  bottom_center = paste0("Last updated: ", Sys.time(), " | ", "Slides available here: github.com/nvandeweerd/ml_seminar_2024_03_15"),
  exclude = "title-slide"
)
```
...
```{r, out.width="60%"}
knitr::include_graphics("figures/id_pic_cropped_small.jpeg")
```
...
.content-box-blue[
`r icons::fontawesome("hand-paper")` How many of you have experience with programming (R, python, etc.,)  
...
`r icons::fontawesome("search")` Check out the `topicmodels` package in R and Murakami et al. (2017). But also `r Citet(bib, c("Shadrova2021a"), before = "", after = "")` for a word of caution.]  
--
...
`r icons::fontawesome("search")` Your research:  
...
`r icons::fontawesome("coffee")` *Coffee break* (15.30)
...
```{r, out.width="70%"}
knitr::include_graphics("figures/excel_clean.png")
```

`SUBSTITUTE()` function (For more details see [here](https://support.microsoft.com/en-au/office/substitute-function-6434944e-a904-4336-a9b0-1e58df3bc332))    
...
.content-box-blue[
`r icons::fontawesome("flask")` Try this: What regular expression could be used to remove the 'headers' from the ICLE texts? (e.g., ` <ICLE-BR-FF-0062.1> `). Be careful not to remove anything else!
]
...
```{r, comment=""}
scan(here("data", "example_texts", "ICLE-TS-NOUN-0005.1_XML.txt"), what = character(), sep = "\n") %>%
  paste0(collapse = " ") %>%
...
icle_txt %>%
  head(20) %>%
  select(idx, sntc, token, tag, lttr, wclass) %>%
  print()
...
`r icons::fontawesome("exclamation-triangle")` Note: For this to work, both TreeTagger and the appropriate tagsets must first be installed locally on your computer. See instructions [here](https://www.cis.uni-muenchen.de/~schmid/tools/TreeTagger/).
...
.content-box-blue[
`r icons::fontawesome("lightbulb")` See [this vignette](https://cran.r-project.org/web/packages/koRpus/vignettes/koRpus_vignette.html) for more information about the `koRpus` package.
]
...
.content-box-blue[
`r icons::fontawesome("github")` Remember that all materials and are sides available on [GitHub](github.com/nvandeweerd/lcr_ss_2023)
...
```{r, out.width="60%"}
knitr::include_graphics("figures/folia_example.png")
```
...
.pull-left[
```{r, comment = ""}
scan(file = here("data", "json_example.json"), what = character(), sep = "\n") 
```
]
...
.pull-right[
.content-box-blue[
`r icons::fontawesome("lightbulb")` Can be [converted](https://www.convertcsv.com/json-to-csv.htm) into CSV/Excel format.  
...
`r icons::fontawesome("exclamation-triangle")` Note: May not be available due to planned outage from June 24th-July 3rd.  
]
]
